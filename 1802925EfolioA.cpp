...
void print_map(const vector<vector<int>>& map, const State& state, int budget) {
    for (size_t i = 0; i < map.size(); ++i) {
        for (size_t j = 0; j < map[i].size(); ++j) {
            ...
            for (const auto& delegation : state.delegations) {
                ...
                if (i == x && j == y) {
                    isDelegation = true;
                    cout << " " << deputies << "D"; // Delegation with deputies and 'D' without space
                    break;
                }
            }

            if (!isDelegation) {
                cout << setw(2) << map[i][j]; // Always show the number of families
                if (isProtected) {
                    cout << "X"; // Add an 'X' to indicate protection
                } else {
                    cout << " "; // Maintain spacing if not protected
                }
            }
        }
        cout << endl;
    ...
    // Calculate total protected families across the whole map for displaying purposes
    int total_protected = calculate_protected_families(map, state);

    cout << "\nFamílias protegidas: " << state.protected_families << "/" << total_protected << endl;
    ...
    int num_deputies = accumulate(begin(state.delegations), end(state.delegations), 0,
                                  [](int acc, const tuple<int, int, int>& delegation) { return acc + get<2>(delegation); });
    cout << "Número de deputados: " << num_deputies << endl;
